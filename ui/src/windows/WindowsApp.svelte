<script lang="ts">
  ...
  function requestAchieve(word: { id: string; displayForm: string }) {
    ...
    achieveError = '';
    achieveDeletion = new Date(Date.now() + (settingsDraft?.achievedRetentionDays ?? 30) * 86_400_000).toLocaleDateString('en');
  }
