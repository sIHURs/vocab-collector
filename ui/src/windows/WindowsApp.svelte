<script lang="ts">
  ...
  import type { CaptureCard, TodayView, WordDetail, WordListItem } from "../lib/types";
...
<div class="windows-shell" data-presentation="windows-main">
  ...
  <main>
    ...
    <section aria-label={`${route} content`}>
      {#if error}<div class="error" role="alert"><span>{error}</span><button onclick={refresh}>Try again</button></div>{/if}
      {#if loading}
        <div class="state" aria-live="polite">Loading your vocabulary...</div>
      {:else if route === "Today"}
        <div class="summary"><div><span>Due today</span><strong>{today?.dueCount ?? 0}</strong><small>{today?.dueCount ? `About ${today?.estimatedMinutes ?? 0} minutes` : "Review queue is clear"}</small></div><div><span>Recent captures</span><strong>{today?.recentCaptures.length ?? 0}</strong><small>Stored locally</small></div></div>
