<script lang="ts">
  ...
  import type { CaptureFailure } from "../lib/types";
  ...
  async function finish() {
    ...
    try {
      ...
      await invoke("capture_ocr_region", { requestId: activeRequest, region: {
      ...
      const failure: CaptureFailure = typeof cause === "object" && cause !== null && "code" in cause && "message" in cause
        ? cause as CaptureFailure
        : { code: "operation", message: cause instanceof Error ? cause.message : String(cause) };
      await invoke("show_region_ocr_failure", { requestId: activeRequest, failure });
    }
  }
...
<style>
  ...
  /* Global button press animations would move and scale viewport coordinates. */
  button, button:active { transform: none; transition: none; }
